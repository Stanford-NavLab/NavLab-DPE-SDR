...
#define STREAMBLOCK_F_L1 (1575.42e6)
#define STREAMBLOCK_F_L2 (1227.60e6)
#define STREAMBLOCK_ANALOG_GAIN (50)
#define STREAMBLOCK_BW_L1 (2.0e6) // smallest BW supported by DBSRX2
#define STREAMBLOCK_BW_L2 (8.0e6)
#define STREAMBLOCK_SETUP_TIME (1.5)
#define STREAMBLOCK_NEW_FILE_CYCLE_MIN (10.0)
#define STREAMBLOCK_NUM_BUFFS (5)

dsp::streamblock::streamblock(
...
/*        const double& rate,
	...
	// Flag all buffers as available for write and set the first one
    _buffs_write_ready = std::vector<bool>(STREAMBLOCK_NUM_BUFFS, false);
    _cur_buff = 0;
    
    // Configure bandwidth for L2 for the corresponding channels
...
int dsp::streamblock::Start(void* cuFlowStream){
    if (_run) return -1;
    _run = true;
    
    // TODO: Launch file write thread
    ...
    _circ_buffs = std::vector<dsp::streamblock::CircBuff<std::vector<std::complex<MEM_TYPE> > > > (
		_chan.size() , dsp::streamblock::CircBuff<std::vector<std::complex<MEM_TYPE> >(_spb)
	);
    
    //_buffs_list = std::vector<std::vector<std::vector<std::complex<MEM_TYPE> > > >(
	//	STREAMBLOCK_NUM_BUFFS, std::vector<std::vector<std::complex<MEM_TYPE> > >(
	//	_chan.size(), std::vector<std::complex<MEM_TYPE> >(_spb) )
	//);
    ...
    //for(auto const& i:STREAMBLOCK_NUM_BUFFS) {
	//	for(auto const& j:_chan){
	//		std::clog<<"\n[UHD] Configuring Channel #" << j << " for buff #" << i << std::endl;
	//		_buff_list_ptrs[i].push_back(&_buffs_list[i][j].front());
	//		set_rf(j,_b_ltwo[j]); // Set filter bandwidth to include L2
	//	}
	//}
...
void dsp::streamblock::set_clock (){
    if (_mimo) {
...
void dsp::streamblock::set_rf(const size_t& ch,const bool is_L2){
...
bool dsp::streamblock::check_locked_sensor(std::vector<std::string> sensor_names, const char* sensor_name, get_sensor_fn_t get_sensor_fn){
